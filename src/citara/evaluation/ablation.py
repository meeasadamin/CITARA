...
def run_configuration(
    ...
    try:
        ...
        row.retrieval_p50_ms = _percentile(latencies, 0.5)
        row.retrieval_p95_ms = _percentile(latencies, 0.95)
    finally:
        retriever.close()
...
def render_markdown(rows: list[AblationRow], gold: GoldSet) -> str:
    ...
    lines = [
        ...
        f"{datetime.now(UTC):%Y-%m-%d}. Every row is the same code path under different "
        "configuration.",
        "",
